...
async function scrapeLuckia() {
    ...
    try {
        ...
        await autoScrollUntilSelectorIsNotFound(page, ".spinnerUpcoming");
        ...
        for (let index = 0; index < sportGroups.length; index++) {
            ...
            for (let index2 = 0; index2 < contests.length; index2++) {
                ...
                try {
                    ...
                    console.log(colors.red(`Error in process contest ${index2}, inside loop: ${error}`));
        ...
        await fs.writeFile(path.join(mockDataDir, jsonDataName), JSON.stringify(allData, null, 2));
...
async function processContest(page, contest, allData, sportName) {
    const eventId = await contest.evaluate((el) => el?.getAttribute("data-event-id") || "N/A");
    if (eventId === "N/A") {
    ...
    const participants = {};
    let markets = [];
    const teamNames = await contest.$$eval(".lp-event__team-name-text, .lp-event__team-name-text--small", (teams) => teams.map((team) => team.innerText));
    ...
    try {
        ...
        if (!parent) {
            console.log(colors.red(`Could not find parent with class lp-event-family for contest ${eventId}`));
            return;
        }
        const categoryName = await parent.$eval(".lp-event-family__name span", (el) => el?.textContent?.trim() || "N/A");
        const categorySubName = await parent.$eval(".lp-event-family__name strong", (el) => el?.textContent?.trim() || "N/A");
        ...
        if (eventHeading) {
            ...
            try {
                ...
                console.log(colors.red(`Timeout waiting for .psk-event-details for contest ${eventId}. Skipping markets for this contest. Error is : ${error}}`));
                return;
            }
        }
        else {
            console.log(colors.red(`No .lp-event__heading found for contest ${eventId}, not possible to click on it`));
            return;
        }
        await page.waitForSelector(".lp-offers__item", { timeout: 15000 });
        markets = await page.$$eval(".lp-offers__item", (markets) => {
            return markets.map((market) => {
                const marketHeadingTitle = market
                    .querySelector(".lp-offer__heading-title")
                    ?.textContent?.trim() || "N/A";
                const picksGroup = market.querySelector(".lp-offer__picks-group");
                const marketsText = {};
                if (picksGroup) {
                    Array.from(picksGroup.querySelectorAll(".lp-offer__pick")).forEach((pick) => {
                        const titleElement = pick.querySelector(".pick-title");
                        const oddElement = pick.querySelector(".lp-offer__pick-content-text");
...
async function goBack(page, eventId) {
    try {
        ...
        if (breadcrumb) {
            ...
            console.warn(`Breadcrumb not found for eventId: ${eventId}, Maybe it was closed before.`);
        }
    }
    catch (error) {
